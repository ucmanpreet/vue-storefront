...
const params: UseUserFactoryParams<any, any, any> = {
  ...
  load: async (context: Context) => {
    ...
    const user_data = context.$bagisto.config.app.$cookies.get('vsf-bagCust-user');
  
    console.log('hiihhihihiihihihih');
    
    console.log({user_data},'USER_DATA_FROM_COOKIES');
    

    if (customerToken) {
      // const loginResult = await context.$bagisto.api.getCustomer(customerToken);
      const loginResult = await context.$bagisto.api.getCustomer(user_data);

      // const customerDetail = loginResult?.data?.accountInfo?.customer;
      const customerDetail = loginResult?.data?.user;
  ...
  register: async (context: Context, params) => {
    ...
    return {
      ...
      success: registerResult?.data?.createUser,
  ...
  logIn: async (context: Context, params) => {
    ...
    const loginResultResponse = {
      key: 'customer_login',
      customerToken: loginResult?.data?.userLogin?.accessToken || null,

      user: loginResult?.data?.userLogin?.user || null,

      success: loginResult?.data?.userLogin?.success,
